...
use bevy::prelude::{
    Component, DetectChanges, EventReader, EventWriter, IntoSystemConfig, OnUpdate, Plugin, ResMut,
    Resource,
};

use super::story::{Character, FontStyle, StoryEvent};
...
impl Plugin for LocationPlugin {
    fn build(&self, app: &mut bevy::prelude::App) {
        app.init_resource::<Region>()
            ...
            .add_system(display_area.in_set(OnUpdate(GameState::InGame)));
    }
}
...
fn display_area(mut story_writer: EventWriter<StoryEvent>, location: ResMut<Region>) {
    if !location.is_changed() {
        return;
    }
    let para = [(location.to_string(), Character::None, FontStyle::None)];
    story_writer.send(StoryEvent::Append(para.to_vec().into()));
}
